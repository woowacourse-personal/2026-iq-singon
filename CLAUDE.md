...
- 유튜브 임베드: 일부 영상은 임베드 비허용(에러 101/150). onError 핸들러 구현됨.
  **실측(2026-07): TJ노래방 공식 MR은 권리사(Ziller-TJ Communication)가 Content ID로 외부 재생을 전면 차단 → 사용 불가.
  금영(KY) 공식 MR(@KARAOKEKY)은 곡별로 재생 가능(최신곡 위주 허용, 구곡 일부 차단).** UI 안내도 금영 기준.
